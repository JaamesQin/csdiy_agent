...
这些修复、账号安全链路及新增在线运行时已有 623 项 Python 自动化测试覆盖，并已通过 v21 新鲜外部全量回归；由于设计
...
不能替代生产授权；能力总览只展示已经上线的能力；代码辅导可生成、解释、诊断、审阅、修复、
重构和设计测试，但始终只做静态检查并返回 `ran_code=false`，语言解析不调用编译器或执行用户代码；课程目录事实来自已校验 registry/
