...
CAPABILITIES: tuple[CapabilitySpec, ...] = (
    ...
    CapabilitySpec(
        ...
        summary=(
            "按要求生成最小完整示例，也可解释、诊断、审阅、修复、重构代码并设计测试；"
            "所有结果都只做静态检查。"
        ),
        usage=(
            "直接说明语言、主题和约束，即可生成一个带预期行为的最小完整示例。",
            "直接粘贴能复现问题的最小代码即可；Markdown 围栏和语言标签是可选的。",
            "可以要求逐行解释、代码审阅、修复、重构，或为代码/规格设计测试用例。",
            "诊断时最好同时提供期望行为、实际行为和原始工具链错误输出。",
        ),
        examples=(
            "用 C++ 写一个展示虚函数、override 和多态调用的最小完整示例。",
            "请分析：\n```cpp\nint main( { return 0; }\n```",
            "重构下面的 Python 函数，并为边界条件设计单元测试。",
        ),
        limitations=(
            "生成代码只做结构化静态检查，不编译、不执行或安装依赖，ran_code 始终为 false。",
            "不会代写可直接提交的完整课程作业答案。",
            "不持久化代码正文；跨会话修改需要重新提供代码。",
        ),
    ),
    CapabilitySpec(
