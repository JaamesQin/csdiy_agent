...
| 编号 | 能力 | MVP 范围 | 验收结果 |
| --- | --- | --- | --- |
| P0-1 | AgentVerse 接入 | 在清小搭中完成一个可使用的 CoursePilot 应用 | 能从平台入口完成一次端到端流程 |
...
| P0-5 | 资料解析与索引 | 首先支持公开 PDF、网页/Markdown，以及用户有权使用的材料 | 检索片段保留来源与页码、标题或时间戳锚点 |
...
### 阶段 1：验证 AgentVerse 最小链路

**目标：** 在投入后端和课程整理前，确认平台能承载最小流程。

**任务：**

1. 创建 CoursePilot 测试应用；
2. 配置开场说明和最小系统提示；
3. 完成“用户输入 → 模型处理 → 输出”的最短链路；
4. 验证资料输入、检索、变量传递、用户文件、状态保存、工具调用和日志能力；
5. 记录每项能力的实测结果和限制；
6. 为不可用能力确定降级方案；
7. 导出或备份第一个可运行版本。
...
**完成标准：** 能从清小搭完成一轮输入和输出；所有 P0 平台依赖均有“支持、降级或阻塞”结论。
...
**完成标准：** 核心 Demo 连续执行 5 次，至少 4 次无需开发者介入即可完成；五类入口均可直接触发；信息完整时不会强制用户从画像或选课重新开始。
...
- 完成阶段 1：AgentVerse 最小链路和能力验证；
- 完成阶段 2：样板课程 Manifest；
- 启动阶段 3：schema 和黄金 StudyKit；
- 建立风险清单和每日阻塞记录。

**周验收：** 平台最短链路可运行；样板课程和核心讲次确定；数据协议不再频繁变化。
...
具体代码目录只有在平台验证表明确需要独立后端时才创建。若 AgentVerse 内置能力足以完成 MVP，优先减少自建基础设施。
...
| 平台与后端负责人 | AgentVerse 编排、状态、工具接入、缓存、部署和故障处理 |
...
1. CoursePilot 已接入 AgentVerse 2.0 并可从清小搭使用；
